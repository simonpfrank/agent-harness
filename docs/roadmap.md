...
### Browser automation (added 2026-08-06, shipped 2026-08-31)
...
**Not a native subsystem after all** — built on Microsoft's official
`@playwright/mcp` server via the existing MCP client support instead,
avoiding the "new subsystem" cost this entry originally assumed. See
"Browser automation" in Done below for the summary, `docs/features.md`'s
"Browser automation" section for the full mechanism, and
`/Users/simonfrank/.claude/plans/as-i-want-a-nifty-rabbit.md` for the full
design discussion (accessibility-tree approach, the security model, and
the real 2026 indirect-prompt-injection research it's grounded in).
...
### Browser automation (added 2026-08-06, resolved 2026-08-31)

Full PRD-then-plan cycle first, not a code-first build — real discussion covering the art-of-the-possible for LLM browser agents (accessibility-tree vs. screenshot approaches, how the field converged on the former in 2026), then a dedicated security discussion once the user raised concerns directly: indirect prompt injection against browser agents is operational in the wild right now (real 2026 payloads found in research — e.g. a fully-specified payment instruction embedded as ordinary-looking page text), not a hypothetical risk to design around defensively "just in case." Full design: `/Users/simonfrank/.claude/plans/as-i-want-a-nifty-rabbit.md`.

**Built on Microsoft's official `@playwright/mcp` server via the existing MCP client support — no native Playwright subsystem**, closing this entry's original "real scope, new subsystem" assumption; adopting the already-hardened accessibility-tree implementation cost far less than building one. New example agent `agents/browser-assistant/`.

**One real architectural gap found during planning, not assumed**: MCP tool exposure was all-or-nothing per server (`runtime.py::_start_mcp_manager` merged every tool a server reported, filtered only by name-collision) — confirmed by reading the code directly before designing around it, not guessed. `@playwright/mcp`'s own `--caps` flag is additive-only and can't exclude core tools like `browser_type`/`browser_run_code_unsafe`. Fixed with an optional per-server `tools: list[str] | None` allow-list on `McpServerSpec` (`mcp_client.py`), enforced during merge in `runtime.py` — `None` (the default) preserves today's all-or-nothing behavior exactly, verified with a regression test against the existing `hello` filesystem-MCP demo.

**A second real gap surfaced during the security discussion, not in the original plan**: off-domain navigation via a link click bypassed domain gating entirely — `browser_click`'s arguments carry an opaque element reference, not a URL, so the harness can't know a click's destination before the call runs (the existing `network_exfiltration_blocker` only inspects pre-call arguments). Closed with a post-call wrapper (`runtime.py::_wrap_browser_nav_tool`) for `browser_click`/`browser_navigate` specifically on a server named `playwright`: after the call, the resulting page's domain is parsed from Playwright MCP's own `"- Page URL: <url>"` result line (verified against a real running server via a throwaway probe script before writing the regex, not assumed) and run through a *synthetic* `browser_navigate` `ToolCall` against the existing hook chain — reusing `network_exfiltration_blocker`'s allowlist/prompt/persistence exactly, zero duplicated logic. Denied → `browser_navigate_back` is called for real and the agent gets a message instead of the unapproved page's content. `browser_navigate` is wrapped too, not just `browser_click` — catches a redirect its own pre-call URL check can't see. Required threading `hooks` into the MCP tool-merge step, which didn't need it before.

**Third addition, folded into v1 scope mid-discussion rather than deferred**: `injection_scanner`'s three keyword patterns ("ignore previous", etc.) would not catch any of the real indirect-prompt-injection payloads found during research — none of them use those phrases, they just look like ordinary page content. Added `tools.py::_strip_hidden_content` (real `lxml` DOM parse, not regex-mangled HTML): removes CSS-hidden elements (`display:none`, `visibility:hidden`, `opacity:0`, the `hidden` attribute) and zero-width Unicode characters before trafilatura ever sees the text — structural pattern-matching, not an NLP classifier, since the hiding technique itself has to exist for the attack to work. Wired into the *shared* `_extract_readable_content` helper, so `web_fetch` benefits too, not just browser-sourced content — broader than the plan's original browser-only framing, with no extra cost once the helper was already shared. A general injection classifier was explicitly not built — disproportionate scope, still bypassable per current research; containment (domain gating, `always_ask` for any future input/commit-shaped tool) remains the real defense.

**`read_live_page_content`** (`tools.py`) reuses `web_fetch`'s trafilatura extraction (refactored into a shared `_extract_readable_content` helper) against the live, JS-rendered DOM via `browser_evaluate` — closes the real gap that `web_fetch`'s plain `httpx.get` can't see past a single-page app's pre-hydration HTML shell. Reaching the live browser session from a built-in tool needed a construction-order fix in `prepare_runtime`: MCP servers now connect (`_connect_mcp_servers`) *before* `build_tool_registry` runs, threading the live `McpManager` through a new `ToolRuntimeContext.mcp_manager` field, with the tool-merge step (`_merge_mcp_tools`) running later as before.

**Profile posture**: ephemeral/incognito by default (`--isolated`), a real/persistent profile only as an explicit per-agent `--user-data-dir` opt-in — a config choice on the MCP server's own launch args, no harness code. **v1 scope, deliberately narrow**: read/navigate only (`browser_navigate`/`browser_click`/`browser_snapshot`/`browser_wait_for`/`browser_tabs`/`browser_close`); form-filling, typed input, file upload, and arbitrary JS execution are not exposed — documented as the pattern to extend (join `permissions.always_ask` regardless of domain trust) rather than built now.

Verified: TDD throughout, red confirmed before every implementation (`tests/unit/test_network.py::TestHasNetworkIntent`, `tests/unit/test_runtime.py::TestMcpWiring`/`TestBrowserNavDomainGuard`, `tests/unit/test_tools.py::TestReadLivePageContent`/`TestStripHiddenContent`). `pytest tests/unit -q` → 845 passed (up from 820); ruff clean; `mypy --strict` clean (43 files, after adding `lxml-stubs` as a dev dependency); `radon cc --min D` → none (the three C-ratings that show up — `validate_config`, `discover_tools`, `_has_network_intent` — are all pre-existing, confirmed via `git stash` before/after comparison on each, not introduced by this work; `_merge_mcp_tools` lands at C(11) after the new branch, within the project's own C-is-acceptable gate). Real, zero-mock end-to-end verification against a live `npx @playwright/mcp@latest` server (2026-08-31, no mocks): domain approval fired once and persisted correctly, didn't re-prompt for an already-approved domain; a real accessibility-tree snapshot confirmed (role/name/`ref` structure, not a screenshot); `read_live_page_content` correctly extracted trafilatura-cleaned text from a real JS-rendered page; a click to a genuinely new domain triggered a fresh prompt, and denying it **actually reverted the live browser** and withheld the unapproved page's content — the hardest and most novel piece of this feature, proven against the real server, not just mocks. `docs/features.md` gained a new "Browser automation" section plus updates to the MCP client support section (the new allow-list), Tools section (`read_live_page_content`), Example agents section, and its "What's explicitly NOT built" line removed. `.gitignore` gained `**/.playwright-mcp/` (a real operational detail found live — the server writes accessibility snapshots to a file in the CWD by default).
