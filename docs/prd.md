...
| **Immutable audit trail** | Paperclip | Append-only log of every tool call, API request, and decision. Cannot be edited or deleted. We have trace files (JSONL) but they're not tamper-evident. Adding a hash chain (~10 lines) would make our traces verifiable. |
| **Per-task cost attribution** | Paperclip | Granular cost tracking — which task/project is expensive? We track per-agent budget but not per-task. Adding a task_id to trace events and budget (~10 lines) would enable this. |
| **Governance with approval gates** | Paperclip | Consequential actions require human approval by decision type, not just by tool. Our permissions are per-tool; Paperclip's are per-decision-severity. Could work as a hook pattern — worth considering if multi-agent orchestration becomes a primary use case. |
| **Shared workspace communication** | Paperclip | Agents read/write to a shared directory for coordination. Our agents use `run_agent` (fire-and-forget). A `workspace/` directory convention would enable richer multi-agent workflows without framework changes. |
