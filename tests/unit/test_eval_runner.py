...
def _mock_runtime(
    response_text: str = "4", turns: int = 1, cost: float = 0.001
) -> MagicMock:
    runtime = MagicMock()
...
class TestRunCaseOnce:
    ...
    def test_basic_run_with_contains_gate(
        self, mock_prepare_runtime: MagicMock
    ) -> None:
    ...
    @patch("eval.runner.prepare_runtime")
    def test_gate_failure_sets_gate_passed_false(
        self, mock_prepare_runtime: MagicMock
    ) -> None:
        mock_prepare_runtime.return_value = _mock_runtime(
    ...
    def test_signal_score_none_when_gate_fails(
        ...
        mock_grade_with_model: MagicMock,
    ) -> None:
        from eval.types import GradeResult

        mock_prepare_runtime.return_value = _mock_runtime(response_text="wrong")
        mock_grade_with_model.return_value = GradeResult(
            passed=True, score=0.9, detail={}
        )
        case = Case(
            id="c1",
            agent=VALID_AGENT,
    ...
    @patch("eval.runner.prepare_runtime")
    def test_applies_instructions_override(
        self, mock_prepare_runtime: MagicMock
    ) -> None:
        mock_prepare_runtime.return_value = _mock_runtime()
        ...
        cell = Cell(
            id="variant-a", agent=VALID_AGENT, instructions_override=variant_path
        )
    ...
    @patch("eval.runner.prepare_runtime")
    def test_applies_provider_model_temperature_overrides(
        self, mock_prepare_runtime: MagicMock
    ) -> None:
        mock_prepare_runtime.return_value = _mock_runtime()
        case = Case(id="c1", agent=VALID_AGENT, prompt="p", graders=[])
        ...
        assert passed_config.model == "gpt-5-mini"
    ...
    @patch("eval.runner.prepare_runtime")
    def test_reads_output_file_when_configured(
    ...
    @patch("eval.runner.prepare_runtime")
    def test_missing_output_file_leaves_content_none(
        self, mock_prepare_runtime: MagicMock, tmp_path: Path
    ) -> None:
        mock_prepare_runtime.return_value = _mock_runtime()
        case = Case(
    ...
    @patch("eval.runner.prepare_runtime")
    def test_permission_prompt_denies_by_default(
        self, mock_prepare_runtime: MagicMock
    ) -> None:
        mock_prepare_runtime.return_value = _mock_runtime()
        case = Case(id="c1", agent=VALID_AGENT, prompt="p", graders=[])
...
class TestRunRepeated:
    @patch("eval.runner.prepare_runtime")
    def test_runs_n_times_with_increasing_index(
        self, mock_prepare_runtime: MagicMock
    ) -> None:
        mock_prepare_runtime.return_value = _mock_runtime()
        case = Case(id="c1", agent=VALID_AGENT, prompt="p", graders=[])
